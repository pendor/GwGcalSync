...
final class Synchronizer extends Thread {
	...
	Synchronizer(final ThreadGroup mainGroup, final Configurator configurator)
			throws Exception {
	...
	final synchronized void calendarChanged(final CachedCalendar calendar) {
  ...
  public final void run() {
		for (;;) {
			try {
				...
				final Calendar newCalendar = ICalUtilities
				...
				if (changes.length == 0) {
					...
					// Get timezones
					final VTimeZone[] timeZones = ICalUtilities
	...
	final byte[] syncronizeNow(final CachedCalendar calendar) throws Exception {
		log.debug("Starting Google Calendar synchronizer...");
		...
		final HashSet processedUids = new HashSet();

		// Parse ics files
		final Calendar localCalendar = ICalUtilities.parseCalendar(calendar.body);
		final Calendar remoteCalendar = ICalUtilities
				.parseCalendar(calendar.previousBody);

		// Get local and remote changes
		final VEvent[] localChanges = ICalUtilities.getNewEvents(remoteCalendar,
				localCalendar, true, calendar.url);
		final VEvent[] remoteChanges = ICalUtilities.getNewEvents(localCalendar,
				remoteCalendar, false, null);

		// Updatable and removable events
		final LinkedList insertableList = new LinkedList();
		final LinkedList updatableList = new LinkedList();
		final LinkedList removableList = new LinkedList();
		...
		if (!removableList.isEmpty() && deleteEnabled) {
			...
			final VEvent[] events = new VEvent[removableList.size()];
		...
		if (!updatableList.isEmpty()) {
			...
			final VEvent[] events = new VEvent[updatableList.size()];
	...
	private final void loadEventRegistry() {
		...
		try {
			if (eventRegistryFile.isFile()) {
				...
				final int len = (int) eventRegistryFile.length();
				byte[] bytes = new byte[len];
				file.readFully(bytes);
				file.close();
				final String content = StringUtils.decodeToString(bytes,
	...
	private final void saveEventRegistry(final String calendarURL, final byte[] newBytes)
			throws Exception {

		// Verify ics file
		final char[] chars = new char[Math.min(newBytes.length, 100)];
		int i;
		...
		// Parse new ics file
		final Calendar newCalendar = ICalUtilities.parseCalendar(newBytes);
		final VEvent[] newEvents = ICalUtilities.getEvents(newCalendar);
		HashMap uids = new HashMap();
		VEvent event;
		String uid;
		...
		try {
			...
			// Write calendar uids
			final Iterator keys = eventRegistry.keySet().iterator();
			Iterator iterator;
			String url;
			while (keys.hasNext()) {
